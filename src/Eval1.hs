...
binaryOperation :: (a -> a -> b) -> Exp a -> Exp a -> State -> Pair b State 
binaryOperation bop e0 e1 s = let (v0 :!: s')  = evalExp e0 s
                                  (v1 :!: s'') = evalExp e1 s'
                              in  ((bop) v0 v1 :!: s'')

unaryOperation  :: (a -> b) -> Exp a -> State -> Pair b State
unaryOperation uop e s = let (v :!: s') = evalExp e s
                         in  (uop v :!: s')
...
evalExp (Div e0 e1)   s = binaryOperation (div) e0 e1 s
